...
def draw_results(detected, input_img, faces, ad, img_size, img_w, img_h, model, time_detection, time_network,
                 time_plot):
    if len(detected) > 0:
...
def main():
    ...
    face_cascade = cv2.CascadeClassifier('./pre-trained/lbpcascade_frontalface_improved.xml')
    ...
    detected_pre = []
    input_img = cv2.imread("./img/origin.jpg")
    ...
    while True:
        ...
        if img_idx == 1 or img_idx % skip_frame == 0:
            ...
            input_img, euler = draw_results(detected, input_img, faces, ad, img_size, img_w, img_h, model, time_detection,
